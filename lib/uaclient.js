...
//    sys.puts("I "+sys.inspect(q));
    for(var i in q.children) {
        if (q.children.hasOwnProperty(i)) {
            ...
            if (t === 'attachment') {
                if (q.annotations === undefined) { q.annotations = [] }
                flatten(c);
                q.annotations.push({
                    "body": c.text,
                    "epoch": c.date,
            ...
            if (t === 'replyby') {
                if (q.replies === undefined) { q.replies = [] }
                flatten(c);
//                sys.puts(sys.inspect(c));
            ...
            if (q.children[i].hasOwnProperty('children')) {
            //    sys.puts("? ignoring tags with children");
            } else {
                if (q.hasOwnProperty(t)) { sys.puts("F would arrayize "+t); }
...
function UAClient(logger) {
 ...
 //           req_user_list(self)
    ...
    self.addListener("parsed", function(j) {
        ...
        // ...and also remove control characters
        j = q1.replace(/\n/g, '\\n').replace(/[\x00-\x1F]/g, '')
        ...
        try {
            ...
            log.critical(sys.inspect(e.message));
            fs.writeFileSync('errordump', j);
